...
import { Send, MessageSquare, Search, ArrowLeft, Loader2, Building2, Plus } from "lucide-react";
...
export default function InsuranceMessages() {
  ...
  const fetchHospitals = async () => {
    try {
      const res = await fetch(`${API_ENDPOINTS.ORGANIZATIONS}?type=hospital`, {
        headers: { Authorization: `Bearer ${token}` },
      }).catch(() => null);
      if (res?.ok) {
        const data = await res.json();
        const orgs = Array.isArray(data) ? data : (data.organizations || []);
        setHospitals(orgs.map((o: any) => ({ id: o._id || o.id, name: o.name })));
      }
    } catch (e) {
        ...
        <div className="bg-white px-6 py-4 flex items-center gap-4 border-b border-gray-200 shadow-sm">
          ...
          <div>
            <h3 className="font-bold text-gray-900">
              {selectedConversation.otherPartyName || "Hospital"}
            </h3>
            ...
            messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.fromUserId === userId ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-sm ${
                  ...
                  <p>{msg.text}</p>
                  <p className={`text-[10px] mt-1 ${msg.fromUserId === userId ? "text-emerald-200" : "text-gray-400"}`}>
        ...
        <div className="bg-white px-6 py-4 border-t border-gray-200">
          <form onSubmit={sendMessage} className="flex gap-3">
            <input
              ...
              className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10"
            />
            <button
              type="submit"
              disabled={!messageInput.trim() || sending}
              className="w-11 h-11 bg-emerald-600 text-white rounded-xl flex items-center justify-center hover:bg-emerald-700 disabled:opacity-50 transition-colors shadow-md"
            >
  ...
  if (showNewChat) {
    return (
      <div className="p-6 bg-gray-50 min-h-screen">
        ...
        <p className="text-sm text-gray-500 mb-4">Select a hospital to message:</p>
        <div className="space-y-3">
          {hospitals.map((h) => (
            <button
              key={h.id}
              onClick={() => startConversation(h.id, h.name)}
              className="w-full text-left bg-white rounded-xl p-4 border border-gray-100 shadow-sm hover:border-emerald-300 hover:shadow-md transition-all flex items-center gap-4"
            >
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Coordinate with hospitals</p>
        </div>
        <button
          onClick={() => setShowNewChat(true)}
          className="flex items-center gap-2 px-4 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-semibold hover:bg-emerald-700 transition-colors shadow-md"
        >
      ...
      <div className="relative mb-5">
        ...
        <input
          ...
          className="w-full bg-white border border-gray-200 rounded-xl py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-12 text-center border border-gray-100 shadow-sm">
          ...
          <p className="text-sm text-gray-400 mb-5">Initiate a chat with a hospital to coordinate claims.</p>
          <button
            onClick={() => setShowNewChat(true)}
            className="bg-emerald-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-emerald-700 transition-colors"
          >
            Start Conversation
          </button>
        </div>
        ...
        <div className="space-y-2">
          {filteredConversations.map((conv) => (
            <button
              ...
              className="w-full text-left bg-white p-4 rounded-xl border border-gray-100 shadow-sm hover:border-emerald-200 hover:shadow-md transition-all flex items-center gap-4"
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                  <span className="font-bold text-gray-900 text-sm">
                    {conv.otherPartyName || conv.participantIds.find((id) => id !== userId) || "Hospital"}
                  </span>
